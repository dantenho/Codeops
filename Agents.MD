# 🤖 AGENTS.MD — SOTA Protocol for Coding Agents

## 🧠 MEMORY & CONTEXT PROTOCOL

### "Read Once, Remember Forever"

Agents must **NOT** constantly re-read this entire `Agents.MD` file. Instead, you must **Internalize** and **Memorize** these protocols.

1. **Check Memory First**: Before starting a task, check your specific memory file at `CodeAgents/Memory/{AgentName}.md`.
2. **Update Memory**: After completing a task, append key learnings, architectural decisions, and context to your memory file.
3. **Training System**: Use the `CodeAgents/Training` system to reinforce your knowledge of these protocols.

### Memory File Structure (`CodeAgents/Memory/{AgentName}.md`)
...
- [Date] Decision: Used Factory Pattern for X because Y.
```
...
### Mandatory Package Managers — DO NOT USE NPM/PIP DIRECTLY
...
```bash
❌ FORBIDDEN: npm install, pip install
✅ REQUIRED:  nvm use + package-lock.json, uv pip install
```
...
|-------------------|---------------|----------|
| **Information Gathering** | ✅ `True` | `ls`, `dir`, `cat`, `grep`, `git status`, `git log` |
| **Environment Setup** | ✅ `True` | `uv pip install`, `nvm use`, `mkdir`, `touch` |
| **Verification** | ✅ `True` | `npm test`, `pytest`, `npm run build` |
| **Destructive/Critical** | ❌ `False` | `rm`, `del`, `git clean`, `git reset`, deployment commands |
...
## 📋 AGENTS COVERED BY THIS PROTOCOL

| Agent | Agent ID (`$agentid`) |
|-------|-----------------------|
| Grok.IA (Cline) | `GrokIA` |
| Gemini CLI Flash 2.5 | `GeminiFlash25` |
| Gemini CLI Pro 2.5 | `GeminiPro25` |
| Gemini Pro 3.0 | `GeminiPro30` |
| Jules | `Jules` |
...
Dependabot
Antigravity
##Write your name as a id and create database
...
```
❌ FORBIDDEN: Duplicate task assignment
❌ FORBIDDEN: Overlapping responsibilities
✅ REQUIRED:  Clear ownership boundaries


## MANDATORY DOCUMENTATION PROTOCOL

### WRITETODO — Every Code Operation Must Include

```
├── [OPERATION_TAG] at comment start ([CREATE], [REFACTOR], [DEBUG], etc)
├── Detailed description of purpose
├── All parameters with types, descriptions, constraints, classe
├── Return values with types and edge cases
├── Exceptions/errors that may be raised
├── Usage examples
├── Complexity analysis (Big O) for critical functions
├── Side effects documentation if apliable
├── Design patterns used (if applicable)
├── Thread safety notes (if applicable)
├── Agent signature: "Agent: {AgentName}"
└── Timestamp: ISO 8601 format
Token Used
Telemetry Log with datasttamp in agent database directory
```
...
└── Composer/
    ├── logs/
    ├── errors/ (Python script with coments, to create reflections
    Reflections Error Database
    Error Handling Log
    Telemetry Log)
    └── analysis/
    MEtrics

####ADICIONAR METRICS, Accuracy Speed Quality Reliability Tokenusage, Error handleing, Unit test, Debug, Code Simulation, Adaptability, task, Contest, lenguage, complexity, performance, scability, verification, instruction, linting, github integration, code structure, githubworkflow and optimization
 Error Reflection, A

 -----------------------------------------
 The model must be training
```
...
Examples:
├── log_2025-12-03T$_$AGENTIDNAME_a1b2c3.json
├── error_2025-12-03T14_$Agentidname$--30-00_d4e5f6.json
└── analysis_2025-12-03T14-30-00_g7h8i9.json
```

---

## 📊 JSON SCHEMAS FOR TELEMETRY
...
## 🐙 GITHUB INTEGRATION & MERGE PROTOCOL
...
**Rationale:** Prevents "couldn't find remote ref" errors and keeps branches local until ready for collaboration.
...
3. **Commit Messages:** Must follow the format: `[AgentName] type: short imperative` (see CONTRIBUTING.md for details).
...
class ConcreteStrategy(BaseStrategy):
    ...
    IF Apliable - Integrate with $FILEPATH LINE
