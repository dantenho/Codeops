...
- If input documents are missing or unreadable, report the specific file error.
- If documents are not in the expected format (e.g., missing EARS syntax in requirements), note this in the evaluation but proceed if possible.
- If scores are identical, provide a tie-breaking rationale.
